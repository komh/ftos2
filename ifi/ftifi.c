...
/* kr_hj2hg :                                                               */
/*                                                                          */
/*   a function to convert Korean Hanja to Hangul of sound of it.           */
                   ...
                   int         index)
{
   ...
   switch (mode) {
      ...
      case TRANSLATE_GB2312:
         COPY("No translation : PM2TT mode = "); CATI( mode ); CAT(" index = "); CATI( index );
         ...
         {
            COPY("Convert 947 glyph(Won) to ASCII 92(backslash)\r\n"); WRITE;
                         ...
                         ULONG        cStart)
{
   ...
   for (faceIndex = 0; faceIndex < file->numFaces; faceIndex++) {
      ...
      /* Note: font encoding must be determined before we start looking for   */
      /*       a font name.                                                   */
      /* If Unicode cmap exists in font and it contains more than 3072 glyphs */
...
static  void LimitsInit(void) {
   max_open_files = PrfQueryProfileInt(HINI_USERPROFILE, "FreeType/2", "OpenFaces", 12 );
   if( max_open_files == 0 )
...
LONG    interfaceSEId(TT_Face face, BOOL UDCflag, LONG encoding) {
    ...
    if (encoding == PSEID_UNICODE) {
       ...
       /* if font is 'small', use PM383; this is done because of DBCS
          systems */
       if (!UDCflag && props.num_Glyphs < AT_LEAST_DBCS_GLYPH) {
